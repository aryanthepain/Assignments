        ...
        column(
            ...
            div(
                ...
                div(
                    ...
                    sliderInput("alpha", "Tail Probability (α):", min = 0.01, max = 0.2, value = 0.05, step = 0.01),
                    radioButtons("sigma_option", "Sigma:",
                        choices = list("Known (Z distribution)" = "known", "Unknown (t distribution)" = "unknown"),
                        selected = "known"
                    ),
                    numericInput("sample_size", "Sample Size (n, only for t-distribution):", value = 30, min = 2),
                    uiOutput("left_shift_slider"), # Dynamic slider for left endpoint
                    helpText("Adjust α to set the confidence level. Choose known/unknown sigma and observe interval comparisons.")
                ),
...
server <- function(input, output, session) {
    ...
    output$left_shift_slider <- renderUI({
        sliderInput("left_shift", "Left Endpoint Shift:",
            min = -3, max = -0.1, value = -critical_value(), step = 0.1
        )
    })
    ...
    output$ci_shifted <- renderText({
        ...
        if (input$sigma_option == "known") {
            ...
            if (cumulative_prob < 1) {
                ...
                "Shifted interval cannot be computed with these parameters. Adjust left endpoint or alpha."
            }
        } else {
    ...
    output$length_comparison <- renderText({
        z_alpha <- critical_value()
        centered_length <- 2 * z_alpha
        left_endpoint <- input$left_shift

        if (input$sigma_option == "known") {
            cumulative_prob <- pnorm(left_endpoint) + (pnorm(z_alpha) - pnorm(-z_alpha))
            if (cumulative_prob < 1) {
                right_endpoint <- qnorm(cumulative_prob)
                shifted_length <- right_endpoint - left_endpoint
                paste(
                    "Centered Interval Length:", round(centered_length, 2),
                    "| Shifted Interval Length:", round(shifted_length, 2),
                    ifelse(shifted_length > centered_length, "(Shifted is longer)", "(Centered is shorter)")
                )
            } else {
                "Unable to calculate shifted interval length due to invalid cumulative probability."
            }
        } else {
            cumulative_prob <- pt(left_endpoint, input$sample_size - 1) +
                (pt(z_alpha, input$sample_size - 1) - pt(-z_alpha, input$sample_size - 1))
            if (cumulative_prob < 1) {
                right_endpoint <- qt(cumulative_prob, input$sample_size - 1)
                shifted_length <- right_endpoint - left_endpoint
    ...
    output$ci_plot <- renderPlot({
        ...
        if (input$sigma_option == "known") {
            ...
            if (cumulative_prob < 1) {
                ...
                plot.new()
                text(0.5, 0.5, "Shifted interval cannot be displayed. Adjust left endpoint or alpha.")
            }
        } else {
            cumulative_prob <- pt(left_endpoint, input$sample_size - 1) + centered_area
            if (cumulative_prob < 1) {
    ...
    plot_interval <- function(crit_value, left_endpoint, right_endpoint, dist_type, df = NULL) {
        ...
        plot(x_vals, y_vals,
            ...
            main = "Effect of Shifting Left Endpoint on Interval Length"
        )
        ...
        abline(v = ci_centered, col = "blue", lty = 2, lwd = 2)
        abline(v = ci_shifted, col = "red", lty = 2, lwd = 2)
